...
class DashboardItem {
  ...
  createMarkup() {
      ...
      `<div class="dashboard__item dashboard__item--${id}">
        <article class="tracking-card">
          <header class="tracking-card__header">
            ...
            <img src="images/icon-ellipsis.svg" alt="menu" class="tracking-card__img">
          </header>
    ...
    this.prev = this.container.querySelector(
      `.dashboard__item--${id} .tracking-card__period`
    );
  }
...
document.addEventListener('DOMContentLoaded', () => {
  getDashboardData()
    .then(data => {
      ...
      const selectors = document.querySelectorAll(".value-person__btn");
      
      selectors.forEach(selector => 
        selector.addEventListener('click', function () {
          selectors.forEach((sel) => sel.classList.remove("value-person__btn--active"));
          selector.classList.add("value-person__btn--active");
